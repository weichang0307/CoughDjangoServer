...
def process_audio(audio_path, sample_rate=None, write_mode="mask", out_dir=None, keep_gain=0.9, apply_energy_gate=True):
    ...
    if write_mode=="mask":
        out=np.zeros_like(y); dur=0.0
        for s,e in segs:
            seg=y[s:e]
            seg=seg if len(seg)<100 else bandpass_filter(seg,sr)
            out[s:e]=seg
            dur+=(e-s)/sr
        if apply_energy_gate: out=_energy_gate(out, sr, segs)
        sf.write(audio_path, out, sr)
        return {"path":audio_path,"sr":sr,"segments":segs,"total_cough_sec":dur,"mode":"mask","energy_gate":apply_energy_gate,"percentile":ENERGY_PERCENTILE}
    elif write_mode=="split":
        if out_dir is None: out_dir=os.path.join(os.path.dirname(audio_path),"cough_segments")
        os.makedirs(out_dir, exist_ok=True)
        written=[]
        for k,(s,e) in enumerate(segs,1):
            seg=y[s:e]
            seg=seg if len(seg)<100 else bandpass_filter(seg,sr)
            if apply_energy_gate: seg=_energy_gate(seg, sr, [(0,len(seg))])
            base=os.path.splitext(os.path.basename(audio_path))[0]
            op=os.path.join(out_dir, f"{base}_cough_{k:02d}_{s}_{e}.wav")
            sf.write(op, seg, sr); written.append(op)
        return {"paths":written,"sr":sr,"segments":segs,"mode":"split","energy_gate":apply_energy_gate,"percentile":ENERGY_PERCENTILE}
    else:
